...
void scanline_convert( struct matrix *points, int i, screen s, zbuffer zb ) {
  ...
  double tx,ty,tz,bx,by,bz,mx,my,mz,tmp;
  int tk, bk, mk;
  tk=i;
  bk=i+1;
  mk=i+2;
  ...
  if(points->m[1][bk]>points->m[1][tk]){
    tmp=tk;
    tk=bk;
    bk=tmp;
  }
  if(points->m[1][mk]>points->m[1][tk]){
    tmp=tk;
    tk=mk;
    mk=tmp;
  }
  if(points->m[1][mk]<points->m[1][bk]){
    tmp=bk;
    bk=mk;
    mk=tmp;
  }
...
void draw_line(int x0, int y0, double z0,
  ...
  if(loop_end-loop_start==0)
    return;
  int diff = C/(loop_end-loop_start);
